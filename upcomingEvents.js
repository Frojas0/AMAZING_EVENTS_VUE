...
const app = createApp({
    ...
    created() {
        fetch(url)
            ...
            .then(data => {
                console.log(data)
                this.upcomingEvents = data.events.filter(evento => evento.date > data.currentDate)
    ...
    computed: {
        filter() {
            let searchFilter = this.upcomingEvents.filter(event => event.name.toLowerCase().includes(this.searchValue.toLowerCase()))
